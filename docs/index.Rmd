...
```{r echo=FALSE, message=FALSE, fig.height=7, fig.width=9}
recent = read_csv('processed/csa-recent-latest.csv') %>% 
  mutate(
    rate.cases = if_else(is.finite(recent.case.rate.100k) | (recent.case.rate.100k < 0), recent.case.rate.100k, 0),
    edited.rate.cases = if_else(rate.cases > 700, 700, rate.cases),
    edited.rate.cases = replace_na(edited.rate.cases, 0),
    alpha = population / 150000,
    alpha = if_else(alpha > 1, 1, alpha),
    popup = glue('{csa.hood.name}: {round(rate.cases)} cases per 1000 people in last two weeks'),
    includes.prisons = if_else(
      csa.hood.name %in% prison.hoods,
      'y',
      ''
...
pal <- colorNumeric("Greens", domain=recent.joined$edited.rate.cases)
...
leaflet() %>%
  ...
  addPolygons(
    data = recent.joined,
    fillColor = ~pal(recent.joined$edited.rate.cases),
    fillOpacity = 0.5,
    color = '#ddd',
    weight = 1,
    popup = recent.joined$popup
  )
```
